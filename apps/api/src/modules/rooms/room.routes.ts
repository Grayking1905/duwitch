...
export async function roomsRoutes(app: FastifyInstance) {
  ...
  app.post('/', { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    const room = await prisma.room.create({
      data: {
        ...
        hostId: userId, maxMembers: body.maxMembers ?? 50,
        tags: body.tags ?? [], isLive: true,
      },
    })
  ...
  app.delete('/:roomId', { preHandler: [app.authenticate] }, async (req, reply) => {
    const userId = (req as any).user.sub as string
    const { roomId } = req.params as { roomId: string }
    const room = await prisma.room.findUnique({ where: { id: roomId } })
    if (!room) return reply.code(404).send({ code: 'NOT_FOUND', message: 'Room not found' })
    if (room.hostId !== userId) return reply.code(403).send({ code: 'FORBIDDEN', message: 'Only the host can end the room' })
