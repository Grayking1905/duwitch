...
## 2026-04-15 - Insecure Placeholder Authentication Tokens

**Vulnerability:** The authentication system was using insecure, unsigned placeholder JWTs in the `AuthService`. These tokens were mere base64-encoded strings and could not be verified by the server's `jwtVerify` middleware, rendering all authenticated routes unusable in a secure manner.
**Learning:** Decoupling business logic (refresh token persistence) from cryptographic operations (JWT signing) is essential when the cryptographic utility (Fastify's `app.jwt`) is only available at the route layer.
**Prevention:** Never use placeholders for security-critical components. Ensure JWTs are cryptographically signed using a secure secret and that the signing logic is correctly integrated where the signing utility is available.
